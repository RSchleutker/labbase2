...
class Request(db.Model, Importer):
    ...
    Notes
    -----
    The whole point about the BaseEntity class is to have a single
    implementation for comments and files that are interesting for all entities.
    Requests are special in this sense as it does not make sense for all kind of
    child classes. Commercially available antibodies or chemicals, for
    instance, will never be requested. However, the BaseEntity class is still
    the least common denominator to all classes, for which requests are of
    interest.
    """
